...
:::details CASの詳細な仕組み
同じパッケージの同じバージョンが複数のプロジェクトで使用されても、実際のファイルはCAS内に1つだけ保存されます。異なるバージョンでもファイル内容が同一なら、やはり1つだけ保存されます。
...
:::message
この仮想ストア構造により、pnpmは**効率性**（ファイル重複の排除）と**厳格性**（ファントム依存の防止）を同時に実現しています。
:::
...
- **シンボリックリンク非対応環境で問題** - React Native、一部のサーバーレス環境など
...
- **こんな時に**：React Native、サーバーレス、既存プロジェクト移行
...
### 🔍 あなたのプロジェクトに最適な`node-linker`を選ぶ
...
graph TD
    ...
    D -->|いいえ| F{{特殊な要件がある？}}
    F -->|サーバーレス<br/>React Native| G[📦 hoistedを選択]
...
`pnpm`の`node-linker`は、単なる設定ではなく、プロジェクトの成功を左右する重要な戦略的選択です。
