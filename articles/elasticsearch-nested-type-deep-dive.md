...
実際のプロジェクトでは、1000件程度のnestedオブジェクトを含む文書で、インデックスサイズが2倍近くになったケースもありました。
...
文書全体を取得して更新するより、Update APIとPainlessスクリプトを使う方が効率的です：
...
### 3. include_in_parentの活用

簡単な検索とnested検索の両方が必要な場合、データを2重に保存する手もあります：
...
インデックスサイズは増えますが、用途に応じて使い分けができます。
